...
@section('content')
    <div class="container mx-auto">
        ...
        <form action="{{ route('backend.monitoring.cvw.store') }}" method="POST">
            @csrf
            <div class="mb-4">
                ...
                <select name="timeline_id" id="timeline_id" class="w-full border border-gray-300 rounded p-2" required>
                    ...
                    @foreach ($timelines as $tmm)
                        <option value="{{ $tmm->id }}">{{ $tmm->time_week }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="plan_kumulatif" name="plan_kumulatif" class="w-full border border-gray-300 rounded p-2" required>
            </div>
            <div class="mb-4">
                <label for="real_kumulatif" class="block text-gray-700">Realisasi</label>
                <input type="number" id="real_kumulatif" name="real_kumulatif" class="w-full border border-gray-300 rounded p-2" required>
            </div>
